...
let observer = new MutationObserver(mutationRecords => {
    if (emptytable != true)
    {
        tar.getElementsByTagName('tbody')[0].addEventListener('click', function(row_target) {
            ...
            let form = document.querySelector('form[name=fishing]');
...
// метод Clear очищает форму и таблицу для обновления данных при выборе другой таблицы или отправки пользователем данных на сервер
function Clear(arg)
{
...
function CreateFormButton()
{
    ...
    document.getElementById('insert').addEventListener('click', function() {
        if (Validateform())
        {
            ...
            let forms = document.forms.fishing;
...
// метод CreateForm создает форму на веб-странице, с которой работает пользователь
function CreateForm(data)
{
    let Form = document.createElement('form');
    Form.setAttribute('name', 'fishing');
...
//метод CreateTable формирует таблицу при её выборе по полученным данным сервера
