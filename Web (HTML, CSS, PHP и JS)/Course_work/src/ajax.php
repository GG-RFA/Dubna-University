<?php
    ...
    if (isset($dataPost->tab))
    {
        ...
        if (isset($dataPost->status))
        {
            ...
            switch($status)
            {
                ...
                case 'consolidated':
                    ...
                    $list_table = mysqli_query($link, $query) or die("Ошибка: " . mysqli_error($link));
                    $rows = array();
                    while ($row = mysqli_fetch_array($list_table))
                    {
                        if ($row[0] != "сводная таблица") {
                            $query = "SELECT * from `$row[0]`";
                            $data = mysqli_query($link, $query);
                            $celltable = mysqli_field_count($link);
                            array_push($rows, $row[0]);
                            array_push($tcolspans, $celltable);
                        }
                    }
                    array_push($thead, $rows);
                    if (isset($dataPost->data[0])) {
                        $updatacons = "SELECT * from `Сводная таблица` WHERE ";
                        for ($num = 0; $num < $count; $num++) {
                            $cell = $thead[0][$num];
                            $value = $dataPost->data[0][$num];
                            $updatacons .= "`$cell`" . " LIKE " . "'$value'";
                            if ($num + 1 != $count) $updatacons .= " AND ";
                            else {
                                if (isset($dataPost->optionSort[0])) {
                                    if ($dataPost->optionSort[0][0] == 'Не выбрано') {
                                        $updatacons .= " ORDER BY `" . $dataPost->optionSort[0][1] . "`;";
                                    } else if ($dataPost->optionSort[0][1] == 'Не выбрано') {
                                        $updatacons .= " ORDER BY `" . $dataPost->optionSort[0][0] . "`;";
                                    } else $updatacons .= " ORDER BY " . "`" . $dataPost->optionSort[0][0] . "`,`" . $dataPost->optionSort[0][1] . "`;";
                                } else $updatacons .= ";";
                            }
                        }
                    }
        ...
        $key = array($rows);
        $rows = array();
        if ($status == "consolidated" && isset($dataPost->data[0])) $query = $updatacons;
        else $query = "select * from `$val`";
        $result = mysqli_query($link, $query) or die("Ошибка: " . mysqli_error($link));
        ...
        for ($rows_num = 0; $rows_num < $rnum; $rows_num++)
        {
            ...
            for ($cell_num = 0; $cell_num < $count; $cell_num++) {
                if ($status == "consolidated") {
                    ...
                    if ($cnum[0] != $tag) {
                        //используем реализованную на языке SQL функцию подсчёта
                        $query = "SELECT getSummary('$tag')";
                        $summary = mysqli_query($link, $query);
                        $inf = array();
                        while ($row = mysqli_fetch_array($summary)) {
                            array_push($inf, "Суммарное количество человек на мероприятиях по № порядковому " . $tag . " : " . $row[0]);
                            array_push($bcolspans, $count);
                        }
                        $tag = $cnum[0];
                        array_push($rows, $inf);
                    }
                ...
                array_push($bcolspans, 1);
            }
            array_push($rows, $cell);
            if ($rows_num + 1 == $rnum && $status == "consolidated") {
                $query = "SELECT getSummary('$tag')";
                $summary = mysqli_query($link, $query);
                $inf = array();
                while ($row = mysqli_fetch_array($summary)) {
        ...
        $tbody = $rows;
        if ($status == "consolidated") {
            $send = array('thead' => $thead, 'tcolspans' => $tcolspans, 'tbody' => $tbody, 'bcolspans' => $bcolspans);
        } else $send = array('thead' => $thead, 'tcolspans' => $tcolspans, 'type' => $type, 'key' => $key, 'tbody' => $tbody, 'bcolspans' => $bcolspans, 'msg' => $msg);
        header('Content-Type: application/json');
